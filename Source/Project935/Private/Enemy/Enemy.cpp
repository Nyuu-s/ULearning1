...
AEnemy::AEnemy()
{
	...
	SensingComponent = CreateDefaultSubobject<UPawnSensingComponent>(TEXT("PawnSensor"));
...
void AEnemy::HandleDamge(float DamageAmount)
{
...
AActor* AEnemy::ChosePatrolTarget()
{
	...
	if (const int32 TargetsNum = ValidTargets.Num() > 0)
	{
		const int32 Selection = FMath::RandRange(0, TargetsNum - 1);
